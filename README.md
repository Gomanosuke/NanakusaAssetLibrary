...
- **右**: 大きなプレビュー、素材情報、タグ、お気に入り。
  ...
  - 3Dモデル・USD: ポリゴン数・頂点数・メッシュ数。USDはさらに、**Proxy（Yes/No）**、**LOD（Yes/No、ありの場合は段階数）**、上方向軸、マテリアル数、サイズ（バウンディングボックス）も表示します。
...
- スキャンは別のプロセスで行い、Houdiniが止まりません。素材の画像生成・情報の取得・proxy/LOD生成も、Windowsのバックグラウンド優先度（CPUだけでなくディスクI/Oも下げる）で動くので、大量の生成中でもパネルの読み込みを妨げません。
