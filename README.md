...
数万件の素材でも軽く動くように作っています（約2.8万ファイルの合成ライブラリーで確認）。

- フォルダーを切り替えると、一覧の枠はすぐに表示され、画像は少しずつ読み込まれます。
- フォルダーのツリーは、スキャン時に作る索引から読みます。パネルを開くたびにディスクをたどりません。
- 素材情報の取得（別プロセスのhython）は、選択が落ち着いてから始めます。矢印キーで素材を送っている間は起動しません。
