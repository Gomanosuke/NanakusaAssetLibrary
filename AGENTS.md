...
- 同名の入口がないフォルダーに複数のUSDレイヤーがある形（例: ErodiumCiconium_6j98t_OL）は、各レイヤーが素材（`core.package_entries`/`is_shared_usd_layer`）。相対パスで同じフォルダーのtexturesを使うため単独移動は拒否し、フォルダー移動だけ許す。パッケージ判定（`is_usd_package`）は「フォルダーと同名の入口」に限る（以前は一覧に出る.usdを全て入口扱いしていた。USD直下の.usdは今も一覧に出さない）。
