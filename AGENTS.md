...
- **`open_catalog`はHoudiniをスクリプトで開かない。** `hou.ui.setSharedLayoutDataSource(hou.AssetGalleryDataSource(...))`は`$HFS/houdini/AssetGallerySourceMenu.xml`・`layout/assetgallery.py`から確認した「正しい」呼び先だが、**実機検証（使い捨てhoudinifx.exe、クリーンな前後比較を複数回）で、この呼び出し単体だけで（ペイン作成なしでも）セッション内の全ネイティブメニュー・ポップアップが（自作パネル自身のメニューを除いて）反応しなくなる（要Houdini再起動）ことを確認済み**。`createFloatingPaneTab`でも`Pane.createTab`（ドッキング）でも同じ。Houdini 22.0.447自体のバグと判断し、`open_catalog`は`hou.ui.displayMessage`で手動での開き方を案内するだけにした（2026-09-23）。似た名前の`setSharedAssetGalleryDataSource(source, gallery_name)`は全くの別物（`'layout'`/`'material'`専用、gallery_name必須）で、これを使う修正は誤り（TypeErrorになるだけ）だったことも確認済み。`tests/test_ui.py`の`test_open_catalog_shows_manual_instructions_instead_of_the_call_that_freezes_houdini`（hou.uiをMagicMockでモック）は「どちらの関数も一切呼ばない」ことだけを固定する。再度スクリプトでの自動化を試す場合は、必ず使い捨てhoudinifx.exeで再検証すること（hython batchではUIフリーズ自体を再現できない）。
...
- 同名の入口がないフォルダーに複数のUSDレイヤーがある形（例: ErodiumCiconium_6j98t_OL）は、各レイヤーが素材（`core.package_entries`/`is_shared_usd_layer`）。相対パスで同じフォルダーのtexturesを使うため、素材としてドラッグされたらフォルダーごと移動し、一緒に動いた素材名をステータス欄に出す（ユーザー指示 2026-09-23。以前の「単独移動は拒否」は撤回）。パッケージ判定（`is_usd_package`）は「フォルダーと同名の入口」に限る（以前は一覧に出る.usdを全て入口扱いしていた。USD直下の.usdは今も一覧に出さない）。
...
- **proxyの点属性は`@P`と`@Cd`（displayColor）だけ**（ユーザー指示 2026-09-24）。UV・法線・不透明度などを足さない。上の階層の材質を受け継ぐproxyは、Materialではない空のScope `NAL_proxy_look`へ束縛して材質なしにする（`_needs_look`/`_bind_look`）。空のMaterialにするとHoudiniのScene Viewが灰色で描き`@Cd`が出ない（0.22.2で実際に起きた。戻さない）。受け継いだ葉の材質の不透明度マップを(0,0)で読み、Scene Viewでproxyが消えていたため（Acer）。空の束縛では継承が止まらない。UsdPreviewSurfaceでdisplayColorを読む材質はhusk（Storm）で黒くなったので使わない。既にproxyがある資産でも、この束縛だけは追加する（`{'restyled': ...}`）。
...
- `lod` / `variant`タグは、生成ジョブの成功時（`set_tag`）に加え、`variant_scan.py`（VariantTagJob、起動時とスキャン後、`vstamp`が古いUSDだけ）がファイルの一番上のプリムのvariant setから同期する（元から入っているLOD / variantにも付く。ユーザー指示 2026-09-23）。テストのパネルでは`ui.LibraryWidget.AUTO_TAGS=False`・`AUTO_SCAN=False`（tests/test_ui.py冒頭）で自動起動を止めている。`proxy`タグも同じ仕組み（purpose=proxyの有無、ProxyJob成功時にも付与）。variant_scanが読む内容を増やしたら`Library.STAMP_VERSION`を上げる。
- Houdiniのセッションで最初に開いたパネルは自動でRescanする（ユーザー指示 2026-09-23。`rescan_on_first_open`で無効化可）。
- テストでジョブを実際に起動させない（ダイアログをpatchしても`queue_*`が本物だと、終了時に実行中のQThreadでhythonが異常終了する: 0xC0000409）。
